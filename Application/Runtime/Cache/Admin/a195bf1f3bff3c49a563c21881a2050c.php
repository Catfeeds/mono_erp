...
var _ACTION_  = '/test/erptest/index.php/Admin/StatisticsManage/statistics_web';
var _PUBLIC_ = '/test/erptest/Public';
var _TIMESTAMP_ = <?php echo time();?>;
var _SELF_ = '/test/erptest/index.php/Admin/StatisticsManage/statistics_web.html';
...
-->
	...
	<table class="table  table-striped table-hover table-condensed" style="border:none;">	
		...
		<?php if(is_array($order_web_day)): foreach($order_web_day as $i=>$vo): ?><tr>
			<td style="color:#03c;border:1px solid #ccc;"><?php echo ($i); ?></td>
			<?php if(is_array($vo)): foreach($vo as $key=>$product_list): ?><td style="border:1px solid #ccc;">
			<table class="statistics_table">
				<tr>
				<td style="text-align:center"><?php echo ($product_list["name"]); ?></td>
				
				<td style="text-align:center"><?php if($product_list[set_sku] != ''): ?>是<?php else: ?>否<?php endif; ?></td>
				<td style="text-align:center"><?php echo ($product_list["discount_price"]); ?></td>
