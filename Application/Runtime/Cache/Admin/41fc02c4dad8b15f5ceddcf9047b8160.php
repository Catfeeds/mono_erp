...
var _ACTION_  = '/test/erptest/index.php/Admin/Productstock/product_stock_apply';
var _PUBLIC_ = '/test/erptest/Public';
var _TIMESTAMP_ = <?php echo time();?>;
var _SELF_ = '/test/erptest/index.php/Admin/Productstock/product_stock_apply.html';
...
<script src="/test/erptest/Application/Admin/View/Default/js/productstock/productstock_ajax.js"></script>
<div class="admin">
			...
			<td colspan="8" class="table_title tab_title">
				<span class="fl icon-minus"> 单件产品申请</span>
                <span class="fr">
					<a href="<?php echo U('/Admin/productstock/product_stock_apply_add');?>" class="icon-plus-circle">添加单件库存申请</a>
				</span>            
			</td>
			</tr>
			<tr class="list_head ct">
                ...
                <td align='center' width="90">状态</td>
                <td align='center' width="100">管理操作</td>
			</tr>
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class='ct'>
                    ...
                    <td class="td_ff"><?php echo ($vo["sta"]); ?> (<?php echo (date("Y-m-d H:i:s",$vo["change_time"])); ?>)
                    </td>
                    <td align='center'>
                    <?php if($vo['status'] == 3): ?><a  href="<?php echo U('/Admin/productstock/product_new_received/',array('id'=>$vo['id'] ));?>">完成收货</a>
                       <?php else: ?>
                         <a <?php if(($vo["status"] == 0) or ($vo["status"] == 8)): ?>href="<?php echo U('/Admin/productstock/product_stock_apply_add/',array('id'=>$vo['id']));?>
                           <?php else: ?>style="color:#CCC;"<?php endif; ?>">修改</a>					
                      | <a <?php if($vo["status"] == 0): ?>href="<?php echo U('/Admin/productstock/product_stock_apply_out/',array('id'=>$vo['id']));?> 
                           <?php else: ?>style="color:#CCC;"<?php endif; ?>">取消申请</a><?php endif; ?>
                      | <a <?php if($vo["pro_change_count"] != 0): ?>href="<?php echo U('/Admin/productstock/product_stock_apply_record/',array('id'=>$vo['id'])); else: ?>style="color:#CCC;"<?php endif; ?>">历史记录</a> 	
                    </td>
