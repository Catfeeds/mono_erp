    ...
    static public function AccessDecision($appName=MODULE_NAME) {
        ...
        if(self::checkAccess()) {
            ...
            if(empty($_SESSION[C('ADMIN_AUTH_KEY')])) {
                ...
                //自定义 验证权限
                $allow_access = true;
				$params = array(&$allow_access,$accessList);
				\Think\Hook::add('rbac_access_decision','Admin\\Behavior\\RbacAccessDecisionBehavior');
				\Think\Hook::listen('rbac_access_decision',$params);
// 				dump($allow_access);exit;
                $_SESSION[$accessGuid] = $allow_access;
                return $allow_access;
                //以下部分无效
    ...
    static public function getAccessList($authId) {
    	...
    	//自定义 授权方式
    	$access = array();
    	$params = array($authId,&$access);
    	\Think\Hook::add('rbac_get_access_list','Admin\\Behavior\\RbacGetAccessListBehavior');
    	\Think\Hook::listen('rbac_get_access_list',$params);
//     	dump($access);exit;
    	return $access;
