 ...
 * @package api.impl
 */
class DecisionPointPractice extends OrderApiPracticeBase implements OrderApiPractice {
    ...
    public function callOrderApi()
    {
		...
		//リクエストデータ作成
        $decisionPointRequestModel = $this->_getDecisionPointRequestModel($reqIdBody->requestId);
        //リクエストデータ作成結果判定
        if (is_null($decisionPointRequestModel)) {
			print("ポイント承認リクエスト情報データ不正。\n");
        	return null;
        }
        ...
        //リクエスト引数の設定
		$param = array('arg0' => $userAuthModel, 'arg1' => $decisionPointRequestModel);
		//API実行
		$body = $this->_client->decisionPoint($param);
		//実行結果判定
    ...
    /**
     * _getDecisionPointRequestModel
     *
     * @access private
     * @param $reqestId
     * @return リクエストデータ
     */
    private function _getDecisionPointRequestModel($reqestId)
    {
    	$soapObj = array();
    	$model = $this->_srvReq;
